...
export type ISkillCategoryCount = { [K in ISkillTag]: number }

export const SKILLS_CATEGORY_NAME_MAP: { [T in ISkillTag]: string } = {
  languages: 'Languages',
...
export const SKILLS_CATEGORY_COUNT_MAP = SKILLS.reduce<ISkillCategoryCount>(
  (skillsAcc, skill) => ({
    ...skillsAcc,
    [skill.tag]: (skillsAcc[skill.tag] ?? 0) + 1,
  })
  , {} as ISkillCategoryCount)
